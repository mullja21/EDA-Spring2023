...
# using " " signals to R that the separator is a space
...
#Exercise 4: Join the temperature and rainfall into one tidy data frame with 3 columns: Date, Rainfall and Temperature. inner_join does this for us. need to have the same number of observations
...
#If you're unsure whether or not there's missing data, then your should examine the first and last observations of your data using filter, sequence and join.
...
```{r}
#Exercise 7: Create a time series objects for each series using ts(). Make sure you specify the starting point and frequency. Repeating data every month, use 12. the frequency depends on the granularity of your data. Weekly data will be 7. We are using 12 here because this is already monthly data/ monthly observations. you can always create a variable that will provide the data that you want. Our example here is the variable, f_month. f_month here is january and f_year is 1901

f_month <- month(first(BR_complete$Date))
f_year <- year(first(BR_complete$Date))

BR_Rainfall_ts <- ts(BR_complete$Rainfall, frequency = 12, start=c(f_year, f_month))

BR_Temperature_ts <- ts(BR_complete$Temperature_C, frequency = 12, start=c(f_year, f_month))

#Check to ensure all of your observations made it into the data set

#Can check the beginning and end of your data set
head(BR_Rainfall_ts)
tail(BR_Rainfall_ts)
...
# Exercise 8: Use the stl() function to decompose both series. Need to have a time series object in order to run the stl function. stl tries to find seasonal and trend component; in order to do that the function needs to know your frequency which is specified in your time series object.
...
# components of each series and turn them into data frames. Could have done these inside a pipe, but decided to do them individually since it's only one function
...
ggplot(BR_Temperature_Components, aes(x=Date)) +
  geom_line(aes(y = Observed), color = "black") +
...
# Exercise 11: Apply one of the trend detection test to the original data set. Remember that the data has seasonal component, which test can be applied in this case? #these kinds of statistical tests require a time series object

temp_trend <- trend::smk.test(BR_Temperature_ts)
summary(temp_trend)

temp_trend
#positive S means you have an increasing trend, so the test statistically proved what we saw in the plot. p value shows that the trend is significant and we'll reject the null hypothesis. If we run the rainfall, we'll see there isn't a trend and it isn't statistically significant
