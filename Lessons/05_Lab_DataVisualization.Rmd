...
```{r formatted text}
#Paste command concatenates two things. Expression lets r know it's an expression and not raw text
#warning is something you want to take a look at, but may not need to fix. You must fix error messages 
#Create a plot
Plot_text <-
...
```{r importing data sets, message=FALSE, warning=FALSE}
#Assign a variable to the processed data folder location
#string that matches data location 
processed_data = "Data/Processed_KEY"

#Read in data
#here always points to where the r project
#more stable than set wd
...
#factors are useful for analyzing and visualizing categorical data 
#strings as factors = true
#Factors can have levels and labels


```{r Plot the number of records by month}
#What kind of values are months?
#glimpse is more friendly when we want to print out things
...
#can convert something with as.factor or factor but factor is more versatile
#we defined levels to the factor
...
#set the levels 
#Scale discrete asks r to show the empty categories
ggplot(PeterPaul.chem.nutrients, 
...
#Tidy up the code
#Labs allows you to clean up your plot and add your own labels
ggplot(PeterPaul.chem.nutrients, 
...
#if you want to wrangle your data before putting into a plot, then you may want to pipe your data into a filter and then pipe that result into ggplot (you'll want to do ggplot last). The code can be as long as it needs to be, but if you tidy it up, then it becomes easier to see where you went wrong.
...
#view enables you to see what you can modify within the theme
#type into console View(theme_wsj())
...
# set points uniformly at 0.5. Alpha sets the transparency at a value between 0 and 1.
#facet wrap allows us to add another dimension to better visualize the data
Surface_Temp_by_Day_Plot <- PeterPaul.chem.nutrients %>% 
  filter(depth == 0) %>% 
  ggplot(aes
         (x=daynum,
           y=temperature_C,
  ...
  labs(
    title = "surface temperature vs day of the year by year",
