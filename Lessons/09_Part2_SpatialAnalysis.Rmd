...
#spatial features are made of geometries, attributes, and coordinate reference systems
```
...
#read the spatial data; don't need strings as factors because this is a binary data set and then we can start to use the dplyr commands and star to manipulate the data set statefp= is the fps code/ unique code for every state
...
#can't do an interactive map in pdf format; must knit an interactive map into an html file
...
upper.neuse_sf <- st_read('./Data/Spatial/NCHUC8.shp') %>% 
  filter(SUBBASIN == 'Upper Neuse') #Filter for just Upper Neuse


#Create a map
mapView(huc8_sf, col.regions = 'green') + mapView(upper.neuse_sf, col.regions ='blue')
...
#Read the 2018 SVI county-level dataset for NC; this is a database join. It looks for similarities in the data set and combines them
...
#Join the SVI attributes to the county spatial features; add new columns with the two fields that are in common (line up the GEOID and FIPS)
...
#If we don't tell the R data set to read in the colclasses, we won't be able to set the data type when r reads in the data set. so we need to tell it to read in as a factor and then group by first 8 characters by creating a column for it. substr specifies a string grabbing from first to 8th value
...
#Join to HUC_8 features; just using data frame commands
...
#Convert all to UTM Zone 17 (crs = 26917); this creates a new set of data frames, but the coordinate columns will all be different; everything will now have planar coordinate systems instead of the geographic coordinates. It's imporant to ensure things are speaking in the same language and using the same coordinate system
...
#Clip the HUC2 data set by the NC State boundary datase
neuse_sf <- huc2_utm %>% 
...
#Select intersecting counties using the `filter()` command. This is the old style.
neuse_intersect_2 <- counties_sf_utm %>% 
  filter(st_intersects(x = ., y = neuse_sf, sparse = FALSE))


#Select intersecting counties using the `filter()` command. This is the new style of doing the same thing as the above and it is a simpler syntax. Can set the predicate to st_touches, st_within, or st_overlaps
...
#view the result of the new way of doing it
