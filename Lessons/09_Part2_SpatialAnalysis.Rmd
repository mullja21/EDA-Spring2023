...
#Create a map; must modify colors in ggplot 
ggplot() +
  geom_sf(data=upper.neuse_sf, aes(fill = ACRES)) +
  scale_fill_distiller(palette = "Y10rRd")

mapView(huc8_sf, zcol = 'ACRES') + mapView(upper.neuse_sf, zcol = 'ACRES')
...
#If we don't tell the R data set to read in the colclasses, we won't be able to set the data type when r reads in the data set. so we need to tell it to read in as a factor and then group by first 8 characters by creating a column for it.
