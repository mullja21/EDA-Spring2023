...
NTL.phys.data.Tuesday.1990thru1999 <-
  filter(NTL.phys.data,year4 %in% (1990:1999) & lakename== "Tuesday Lake") 
...
NTL.phys.data.Tuesday.July.processed <- 
  NTL.phys.data %>%
  filter(lakename == "Tuesday Lake" & year4 %in% (1990:1999) & month(sampledate) ==7)
...
NTL.phys.data.Tuesday.July.EX4 <- 
  NTL.phys.data.Tuesday.July.processed %>%
  filter(depth ==0) %>%
  drop_na(temperature_C) %>%
  summarize(mean_temp <- mean(temperature_C))

#only have one value because we are summarize. summarize function actually performs operation on data frame wherease summary only provides overview but doesn't transform the data
...
#Exercise 6: Spread temperatureC into more than one column based on the depth. When we make the dataset wider, we will create more N/As
NTL.phys.data.EX6 <-
  NTL.phys.data %>%
