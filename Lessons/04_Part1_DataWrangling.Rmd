...
#head shows us the first few rows in the data set
head(NTL.phys.data)
...
```{r, results = "hide"}
#shows intro to dplyr tab and how the verbs are organized. Rows, then use filter, slice, or arrange. If columns, then use select, rename, mutate (create a new column based off of existing columns), or relocate. Groups of rows then use sumarise- collapse group into a single row
vignette("dplyr")
...
filter and compare your value to a specific value, then use 
`==`
...
`>`
`>=`
multiple conditions then use the ampersand to aggregate conditions if you want all of them to be true
`&`
If one or the other condition should be true
`|`
...
# matrix filtering; filter only the rows that have zero in the depth column
NTL.phys.data.surface1 <- NTL.phys.data[NTL.phys.data$depth == 0,]

# dplyr filtering
#filter only the rows that have zero in the depth column but using the dplyr function instead of the matrix filtering
NTL.phys.data.surface2 <- filter(NTL.phys.data, depth == 0)

#filter original data set and include a depth less than 0.25
NTL.phys.data.surface3 <- filter(NTL.phys.data, depth < 0.25)

# check to see if the methods arrive at the same result
...
#selecting paul or peter lake
NTL.phys.data.PeterPaul1 <- filter(NTL.phys.data, lakename == "Paul Lake" | lakename == "Peter Lake")
#can also exclude other lakes we aren't observing instead of selecting to isolate the two columns you want
...
#remember filtering only performs tasks on the columns not the rows
#%n% useful for when you need to select for multiple things- this allows you to list them instead of writing all of them out
NTL.phys.data.PeterPaul3 <- filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake"))


# Choose a range of conditions of a numeric or integer variable. Selecting a specific range. the comma selects both
...
NTL.phys.data.1990 <- filter(NTL.phys.data, year4 == 1990)
...
NTL.phys.data.1990thru1999 <- filter(NTL.phys.data, lakename =='Tuesday' & year4 %in% c(1990:1999))
...
```{r}
#whatever is left of the equal sign will be the name of the new column
...
```{r}
# add a month column to the dataset; function month extracts month from data frame
NTL.phys.data.PeterPaul1 <- mutate(NTL.phys.data.PeterPaul1, month = month(sampledate)) 

# reorder columns to put month with the rest of the date variables. select can also be used to change the ordering of the columns
