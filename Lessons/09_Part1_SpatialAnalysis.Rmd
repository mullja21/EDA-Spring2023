...
#csv files cannot store spatial objects, so we have to use st_read to read spatial data frames with geometries
#Can still use joins to combine data sets- it becomes a data frame operation
#spatial aggregation is groub_by() and summarize()
#coordinate system transformations st_transform()
#intersecting data use st_filtered(...,..pred) select all the rows that meet the spatial criteria. pred is the predicate and the default is boarder intersections. This function just selects the data and doesn't manipulate it 
#clipping data use st_intersection()
#spatial selection st_intersects()
...
#Upgrade to a simple feature geometry column by adding a crs; 4326 is a database number from epsg.io which describes the position in the specific coordinate system
...
#Buffer Grainger Hall 1 unit. the 1 refers to degrees of lat and long; so 1 degree of each; buffering it in spherical coordinates
...
#Transform the datasets to use UTM 17 N, WGS84; this will enable us to measure the distance in meters
EPAair.sf.utm <- st_transform(EPAair.sf,32617)
GasPlants.utm <- st_transform(GasPlants,32617)


#Have a look at the coordinate associated with the geometries; confirm that its 
EPAair.sf.utm$geometry[1]

#Plot the data; Axis are still written in degrees, but it's projected and that's why it looks a little distorted 
ggplot() +
  geom_sf(data = EPAair.sf.utm,color='blue') + 
  geom_sf(data = GasPlants.utm,color='red',shape='O') 

#Create a distance matrix between the point in each dataset; pariwise creates matrix
distance_matrix <- st_distance(EPAair.sf.utm, GasPlants.utm, pairwise=T)

#Compute the mean of each row (each EPA site) rowmeans reduces the dataset to means of the rows
mean_distances <-  rowMeans(distance_matrix)

#Add as a column to the epa sites; bind the mean distance to utm
