...
ggplot(GaringerOzone_clean) +
  geom_line(aes(x = Date, y = Daily.Ozone.clean), color = "black") +
...
#group by month first, not by year in the time series on number 9. If the data is out of order, the time series will look like a hump.
...
GaringerOzone.monthly <- GaringerOzone.monthly %>%
  group_by(Year, Month) %>% 
  summarise(Monthly.Ozone = mean(Daily.Max.8.hour.Ozone.Concentration))
...
GaringerOzone.monthly.ts <- ts(GaringerOzone.monthly$Monthly.Ozone, frequency = 12, start=c(f_year_M, f_month_M))
...
> Answer: Initial p value is slightly significant
...
> Answer: #when you decompose a time series, you have the trend, seasonal, ____, and randomnesss. Subtract just the seasonal component and run the Mann Kendall.You've done everything right if your final p value is 0.0075. When you do remove the seasonal, you may have a significant trend. With the seasonal you may not have a significant trend. When you remove seasonal, the p-value becomes even more significant.
