...
> ANSWER: EPSG:4269, North_American_Datum_1983. It is a geographic CRS
> not a projected one.
...
NWIS.df <- read.csv("./Data/Raw/NWIS_SiteInfo_NE_RAW.csv", colClasses = c('site_no' = 'factor')) 
...
ggplot() + 
  geom_sf(data=counties.NB_sf, color = 'darkblue') + 
  geom_sf(data=NWIS.sf, color = 'darkgreen') +
  labs(title = "NWIS Gage Locations in Nebraska EPGS 4269", subtitle = "Jordan Mullens",
       x = "Latitude",
       y = "Longitude") 
...
NB.flow <- read.csv("./Data/Raw/NWIS_SiteFlowData_NE_RAW.csv", colClasses = c('site_no' = 'factor')) %>%
...
ggplot() + 
  ...
  labs(title = "NWIS Gage Locations in Nebraska EPGS 32614", subtitle = "Jordan Mullens",
       x = "Latitude",
       y = "Longitude") 
...
#24 Plot
mapview(counties.NB_utm, col.regions = 'cornsilk4')+ mapview(Douglass_sf, col.regions = 'lightskyblue') + mapview(NWIS.Douglas, col.regions = "darkgreen")
