...
kable(summary_PM25,caption = "Concentration by County: Mean PM2.5")
kable(summary_Ozone,caption = "Concentration by County: Mean Ozone")
...
```{r plot.PM2.5, echo=FALSE, fig.align='left', fig.cap="Mean PM2.5 by Month", fig.height=2.5, message=FALSE, warning=FALSE}

box.plot.PM2.5 <- epa_data %>% 
  drop_na(PM2.5) %>% 
  ggplot( 
  aes(x=factor(Month),y=PM2.5)) +
  geom_boxplot() +
  labs(
    title ='Mean PM2.5 Concentration by Month',
    x = 'Month',
    y = 'PM2.5 Concentration (ug/m3 LC)')
print(box.plot.PM2.5)
```

```{r plot.ozone, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Mean Ozone by Month", fig.align='left', fig.height= 2.5 }

box.plot.Ozone <- epa_data %>% 
  drop_na(Ozone) %>% 
  ggplot( 
