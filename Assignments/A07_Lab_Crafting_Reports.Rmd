...
1.  Rename this file `JordanMullens_A07_CraftingReports.Rmd` (replacing `<FirstLast>` with your first and last name).
...
kable(summary_PM25,caption = "Concentration by County: Mean PM2.5", col.names= c("County", "$\\mu g/m^3$"))
kable(summary_Ozone,caption = "Concentration by County: Mean Ozone",  col.names= c("County", "$\\mu g/m^3$"))
...
```{r plot.PM2.5, echo=FALSE, fig.align='left', fig.cap="Mean Ozone by Month in 2018", fig.height=2.5, message=FALSE, warning=FALSE}

box.plot.Ozone.2018 <- epa_data %>% 
  filter(Year==2018) %>%
  drop_na(Ozone) %>% 
  ggplot( 
  ...
  geom_boxplot() +
  labs(
    title ='Mean Ozone Concentration by Month in 2018',
    x = 'Month',
    y = 'Ozone Concentration (ppm)')
print(box.plot.Ozone.2018)

```

```{r plot.ozone, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Mean Ozone by Month in 2019", fig.align='left', fig.height= 2.5 }

box.plot.Ozone.2019 <- epa_data %>% 
  filter(Year==2019) %>%
  drop_na(Ozone) %>% 
  ggplot( 
  aes(x=factor(Month),y=Ozone)) +
  geom_boxplot() +
  labs(
    title ='Mean Ozone Concentration by Month in 2019',
    x = 'Month',
    y = 'Ozone Concentration (ppm)')
print(box.plot.Ozone.2019)
...
<https://github.com/mullja21/EDA-Spring2023.git>
